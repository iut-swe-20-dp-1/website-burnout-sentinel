  ...
  const handleFileChange = (e) => {
    ...
    if (selectedFile) {
      ...
      reader.onload = (event) => {
        ...
        console.log("liness : ", Number(lines.length));

        if (Number(lines.length) < 40 || Number(lines.length) > 15000) {
          console.log("gorib csv");
          setMessage(
          ...
          setMessage(
            `Received ${selectedFile?.name} with ` + lines.length + ` rows.`
          );
                    ...
                    <form onSubmit={goToTestForm}>
                      <div className="w-full flex items-center justify-between mb-3">
                        <label
                          ...
                          className="flex items-center w-full border border-1 border-gray-300 p-1 rounded-sm"
                          onClick={(e) => e.target.control.click()}
                        >
                          <input
                            type="checkbox"
                            id="show_password"
                      ...
                      <p className="text-lg">
                        You can download a sample CSV to check the expected
                        format from{" "}
                        <Link to={`${sampleCsvUrl}`}>
                          <span className="font-bold text-underline">here</span>
                        </Link>
                        .
                      </p>
