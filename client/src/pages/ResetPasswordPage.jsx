...
const ResetPasswordPage = () => {
    ...
    const handleSubmit = (e) => {
        ...
        console.log(formData.currPassword)
        ...
        <>
            ...
            >
                ...
                <div className="flex justify-center my-2 mx-4 md:mx-0">
                    <form className="w-full max-w-4xl bg-white/80 rounded-lg shadow-md px-12 py-6 my-8 backdrop-blur-sm" onSubmit={handleSubmit}>
                        <div className="text-center mb-8">
                            <h2 className="text-6xl font-bold text-[#7366FF] tracking-tight">
                                Reset Your Password
                            </h2>
                        </div>
                        <LottieAnimation lottie_animation_data={ResetPasswordAnimation} style_classes={"w-2/6 mx-auto"} />
                        <div className="flex flex-wrap -mx-3 mb-6">
                            <div className="w-full md:w-full px-3 mb-6">
                                <label className={`${labelClass}`} htmlFor='currPassword'>Current Password</label>
                                <input className={`${inputFieldClass}`} type={showPassword ? 'text' : 'password'} name="currPassword" value={formData.currPassword} onChange={handleInputChange} required />
                            </div>
                            <div className="w-full md:w-full px-3 mb-6">
                                <label className={`${labelClass}`} htmlFor='newPassword'>New Password</label>
                                <input className={`${inputFieldClass}`} type={showPassword ? 'text' : 'password'} name="newPassword" value={formData.newPassword} onChange={handleInputChange} required />
                            </div>

                            <div className="w-full md:w-full px-3 mb-6">
                                <label className={`${labelClass}`} htmlFor='confirmNewPassword'>Confirm New Password</label>
                                <input className={`${inputFieldClass}`} type={showPassword ? 'text' : 'password'} name="confirmNewPassword" value={formData.confirmNewPassword} onChange={handleInputChange} required />
                            </div>
